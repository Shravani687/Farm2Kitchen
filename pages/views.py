...
@user_passes_test(is_superuser_check, login_url='home')
def admin_dashboard(request):
    # 1. Active Farmers & Hotels Count
    active_farmers = Farmer.objects.count()
    active_hotels = Hotel.objects.count()
    
    # 2. Total Revenue (FIXED)
    # Since 'total_amount' is not a DB column, we fetch the orders and sum them in Python
    delivered_orders = Order.objects.filter(status='Delivered')
    
    # Calculate sum using a generator expression (Efficient)
    total_revenue = sum(order.total_amount for order in delivered_orders)
    
    # 3. Net Profit (10% of Revenue)
    platform_earnings = float(total_revenue) * 0.10

    # 4. Chart Data (Revenue per month)
    # We need a slightly more complex query for charts since we can't use Sum('total_amount')
    # For now, let's keep it simple to prevent errors
    chart_labels = [] 
    chart_data = []
...
from django.views.decorators.csrf import csrf_exempt
import json
from .ai_service import get_gemini_response # Import the file we just made

@csrf_exempt # Disables CSRF for this specific view (easier for hackathon testing)
def ask_ai_assistant(request):
    if request.method == "POST":
        try:
            ...
            # In a real app, you would fetch real DB data here
            # For now, we simulate "Context Data" to make the AI smart
            market_context = "Tomatoes are in high demand. Average price is 20-25 Rs/kg. Onions are oversupplied."

            # Call our AI function
            ai_answer = get_gemini_response(user_question, market_context)
...
# views.py (Add to bottom)
from django.http import JsonResponse
import json
from .ai_service import get_gemini_response # Import the file we just made

def ask_ai_endpoint(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
